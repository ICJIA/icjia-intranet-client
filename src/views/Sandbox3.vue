      ...
      <v-row>
        <v-col class="text-center" style="margin-top: 100px">
          ...
          <button id="clap" class="clap">
            ...
            <span id="clap--count-total" class="clap--count-total mb-10">{{
              numberOfClaps
            }}</span>
          </button>
          <div
            v-if="numberOfClaps > 0"
            style="font-size: 10px; margin-left: -10px"
            class="mt-5"
          >
            You've given this page {{ numberOfClaps }} clap{{
              numberOfClaps > 1 ? "s" : null
            }}.{{
...
export default {
  ...
  mounted() {
    ...
    const maxedOutTimeline = new window.mojs.Timeline();
    maxedOutTimeline.add([
      triangleBurst,
      circleBurst,
      countTotalAnimation,
      scaleButton,
    ...
    function repeatClapping() {
      if (vm.numberOfClaps < vm.clapMax) {
